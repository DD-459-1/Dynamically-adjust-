...
class DA_Adam(Optimizer):
    ...
    def __init__(self, params, batch_num, lr = required, adjustment_rate = 0.3, beta_1 = 0.9, beta_2 = 0.999,
        ...
        defaults = {
            ...
            # 'beta_2_ini': 1 - torch.sqrt(torch.tensor(1 - beta_1_ini)),
            # 'beta_2_ini':  torch.pow(torch.tensor(beta_1_ini), 2),
            'beta_2_ini':  torch.pow(torch.tensor(beta_1_ini),0.9),
            # 'beta_2_ini':  2 * beta_1_ini - torch.pow(torch.tensor(beta_1_ini), 2),
        ...
        super(DA_Adam, self).__init__(params, defaults)
    ...
    def _init_state(self,p):
        ...
        # state["adj_enable"] = True
        # state["trigger_count"] = 0
        # state["beta_1_iter"] = None
        # state["beta_2_iter"] = None
        return state
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            if group['trigger_count'] < adjustment_step:
                if group['adj_enable']:
                    ...
                    if seed < group['probability']:
                        ...
                        # group['beta_1_iter'],group['beta_2_iter'] = self._adjust_beta_iter(beta_1, beta_1_ini)
            ...
            for i, p in enumerate(group['params']):
                 ...
                 velocity.mul_(beta_2).addcmul_(1 - beta_2, grad, grad)
